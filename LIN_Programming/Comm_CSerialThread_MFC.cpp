...
BOOL Comm_CSerialThread_MFC::OpenPort(CString m_stPort, CString m_stBaud, CString m_stDatabit,
	CString m_stFlow, CString m_stParity, CString m_stStop)
{
	...
	// 포트 감시 쓰레드 생성.
	m_bConnected = TRUE;
	SendMessage(this->hCommWnd, WM_DEVICE_HDLR_PORT_CONNECTED, (WPARAM)m_hComm, 0);   // $ sjkim
...
// 포트를 닫는다.
void Comm_CSerialThread_MFC::ClosePort()
{
	m_bConnected = FALSE;
	...
	SendMessage(this->hCommWnd, WM_DEVICE_HDLR_PORT_CLOSEED, (WPARAM)m_hComm, 0);   // $ sjkim
}

// 포트에 pBuff의 내용을 nToWrite만큼 쓴다.
// 실제로 쓰여진 Byte수를 리턴한다.
DWORD Comm_CSerialThread_MFC::WriteComm(BYTE *pBuff, DWORD nToWrite)
{
...
// 실제로 읽혀진 Byte수를 리턴한다.
DWORD Comm_CSerialThread_MFC::ReadComm(BYTE *pBuff, DWORD nToRead)
{
...
DWORD	ThreadWatchComm(Comm_CSerialThread_MFC* pComm)
{
	DWORD		dwEvent;
	OVERLAPPED	os;
	BOOL		bOk = TRUE;
	...
	while (pComm->m_bConnected)
	{
		...
		if ((dwEvent & EV_RXCHAR) == EV_RXCHAR)
		{
			...
			if (!(pComm->m_bReserveMsg))
			{
				...
				::PostMessage(pComm->hCommWnd, WM_DEVICE_HDLR_PORT_RECEIVE, (WPARAM)pComm->m_hComm, (LPARAM)pComm->m_wPortID);   // $ sjkim 
...
Comm_CSerialThread_MFC::Comm_CSerialThread_MFC()
{
	m_bConnected = FALSE;
	m_bReserveMsg = FALSE;
	hCommWnd = NULL;
...
void Comm_CSerialThread_MFC::f_send(CString str)
{
	CString sBuf;
	sBuf.Format("%c", 0x13);
	str += sBuf;

	this->WriteComm((BYTE *)str.GetString(), str.GetLength() + 1);
}




CStringArray* Comm_CSerialThread_MFC::GetPorts() {
