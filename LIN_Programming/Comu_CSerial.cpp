//
// Serial.cpp: implementation of the Comu_CSerial class.
//
...
void Comu_CSerial::ErrorMsg(DWORD dwError)
{
	char msg[512];
	LPTSTR lpMsgBuf;
	...
	sprintf_s(msg, "Errno:[%d]\n%s", dwError, lpMsgBuf);
	MessageBox(NULL, msg, "Comu_CSerial Message", MB_OK);
	LocalFree(lpMsgBuf);
}

BOOL Comu_CSerial::InitSerialEvents()
{
...
BOOL Comu_CSerial::CreateOvlapEvent(OVERLAPPED *pov)
{
	if (pov->hEvent != NULL)
...
void Comu_CSerial::InitOverlapped(OVERLAPPED *pov)
{
	pov->hEvent = NULL;
	pov->Internal = 0;
	pov->InternalHigh = 0;
...
//
BOOL Comu_CSerial::OpenPort(LPSTR szPortName, DWORD dwBaudRate, UINT nDataBit, LPSTR szParity, LPSTR szStopBit)
{
	if (IsOpened())
		return TRUE;

	if (strlen(szPortName) > 4)	// Greater than COM9 (ex: COM10)
		sprintf_s(m_szPortName, "\\\\.\\%s", szPortName);
	else
		sprintf_s(m_szPortName, "%s", szPortName);
...
BOOL Comu_CSerial::OpenPort(LPSTR szPortName, DCB* pdcb)
{
	if (IsOpened())
		return TRUE;
...
///////////////////////////////////////////////////////////////////////////
// 1 byte를 Write 한다.
BOOL Comu_CSerial::WriteSingleByte(BYTE OneByte, UINT nTimeout)
{
	DWORD	dwWritten = 0;
...
UINT Comu_CSerial::WriteBytes(BYTE *pBytes, UINT nLen, UINT nTimeout)
{
	DWORD	dwWritten = 0;
	BOOL	bWrittenDone = FALSE;
...
UINT Comu_CSerial::WriteData(BYTE *pBuff, UINT nLen, UINT nTimeout)
{
...
UINT Comu_CSerial::WriteDataMulti(BYTE *pBuff, UINT nLen, UINT nTimeout)
{
	DWORD	dwTotalWritten = 0;
	BOOL	bWrittenDone = FALSE;
	UINT	i = 0;
...
UINT Comu_CSerial::GetInQueDataSize()
{
	DWORD	dwInQue;
	DWORD	dwError;
	COMSTAT	comstat;
...
///////////////////////////////////////////////////////////////////////////
// 1 byte를 Read 한다.
BOOL Comu_CSerial::ReadSingleByte(BYTE *OneByte, UINT nTimeout)
{
	DWORD	dwReaded = 0;
...
///////////////////////////////////////////////////////////////////////////
// nLen bytes를 Read 한다.
UINT Comu_CSerial::ReadBytes(BYTE *pBytes, UINT nLen, UINT nTimeout)
{
	DWORD	dwReaded = 0;
	BOOL	bReadDone = FALSE;
	//DWORD	dwRes;
...
void Comu_CSerial::StopReading()
{
	SetEvent(m_hStopReading);
}

UINT Comu_CSerial::ReadData(BYTE *pBuff, UINT nLen, UINT nTimeout)
{
	BYTE	btTemp[DEFAULT_SIZE];
	DWORD	dwReaded = 0;
	UINT	nTotalReaded = 0;
...
UINT Comu_CSerial::ReadData2(BYTE *pBuff, UINT nLen, UINT nTimeout)
{
	BYTE	btTemp[DEFAULT_SIZE];
	DWORD	dwReaded = 0;
	UINT	nTotalReaded = 0;
...
UINT Comu_CSerial::ReadDataMulti(BYTE *pBuff, UINT nLen, UINT nTimeout)
{
	BYTE	btTemp[DEFAULT_SIZE];
	DWORD	dwReaded = 0;
	UINT	nTotalReaded = 0;
...
UINT Comu_CSerial::ReadDelayData(BYTE *pBuff, UINT nLen, UINT nTimeout, UINT nRetryCnt, UINT nDelayTime)
{
	BYTE	btTemp[DEFAULT_SIZE];
	DWORD	dwReaded = 0;
	UINT	nTotalReaded = 0;
...
BOOL Comu_CSerial::WaitEventThread()
{
	HANDLE		hThread = NULL;
	DWORD		dwThreadId;
...
///////////////////////////////////////////////////////////////////
// Thread Functions
DWORD WINAPI Comu_CSerial::WaitEventFunc(LPVOID pParam)
{
	Comu_CSerial *p = (Comu_CSerial *)pParam;
...
void Comu_CSerial::SaveLog(char *szDevice, char *szLogData)
{
	return;
	...
	sprintf_s(szTime, "%4d/%02d/%02d %02d:%02d:%02d", today->tm_year + 1900, today->tm_mon + 1, today->tm_mday, today->tm_hour, today->tm_min, today->tm_sec);
	sprintf_s(szDate, "%4d%02d%02d", today->tm_year + 1900, today->tm_mon + 1, today->tm_mday);

	memset(szFilename, 0x00, sizeof(szFilename));
	//sprintf(szFilename, "%s\\Serial_%s%s.log", LOG_DIRECTORY, szDevice, szDate);
	sprintf_s(szFilename, "%s\\Serial_%s%s.log", ".", szDevice, szDate);
